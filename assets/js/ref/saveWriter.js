...
class BinaryWriter {
    ...
    checkDataSize(bytes) {
        if (this.remainingBytes < bytes) {
            const extensionsRequired = Math.ceil((this.byteOffset + bytes) / this.dataExtensionLength) - this._data.byteLength / this.dataExtensionLength;
