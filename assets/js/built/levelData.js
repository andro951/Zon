...
Zon.LevelData = class {
    ...
    static {
        ...
        console.log("blockHealthMultiplePerPrestige: " + this.blockHealthMultiplePerPrestige);
    ...
    getRandomPngFile() {
        ...
        this.levelName = this.texture.name;
        this.width = this.texture.width;
        this.height = this.texture.height;
        this.pixels = this.texture.pixels;
    }
    ...
    //     for (let stageId = Zon.StageID.NONE; stageId < Zon.StageID.COUNT; stageId++) {
    //         const path = `textures/levels/${stageId}`;
    //         Zon.LevelData.allLevelTextures[stageId] = Resources.loadAll(path);
    //     }
    // }
    static async preSetLoadedValuesSetup() {
        ...
        for (let stageId = Zon.LevelData.startingStage; stageId <= Zon.LevelData.maxStage; stageId++) {
            ...
            try {
                ...
                const texturePromises = manifest.files.map(filename =>
                    Zon.LevelData.loadImageWithPixels(`${folder}/${filename}`)
                );

                Zon.LevelData.allLevelTextures[stageId] = await Promise.all(texturePromises);
            } catch (err) {
                console.error(`Failed to load manifest for stage ${stageId}:`, err);
                Zon.LevelData.allLevelTextures[stageId] = [];
            }
        }
    }
